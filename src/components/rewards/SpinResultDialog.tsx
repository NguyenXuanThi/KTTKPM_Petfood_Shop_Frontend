...
export function SpinResultDialog({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isCoin ? "Chúc mừng!" : "Bạn nhận được coupon!"} size="md">
      <div className="relative overflow-hidden text-center">
        ...
        <p className="mx-auto mt-3 max-w-sm text-gray-500 dark:text-gray-400">
          {isCoin
            ? `Bạn nhận được ${formatCoins(result.coinAmount)} xu. Số xu đã được cộng vào tài khoản của bạn.`
            : "Coupon đã được thêm vào tài khoản của bạn và có thể dùng ở My Coupons."}
        </p>
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
          {isCoin && canSpinAgain && onSpinAgain && (
            <Button onClick={onSpinAgain}>
              <RotateCcw size={16} /> Quay tiếp
            </Button>
          )}
          {isCoin ? (
            <Link to="/rewards/shop" onClick={onClose}>
              <Button variant="outline" className="w-full">
                <ShoppingBag size={16} /> Đổi coupon
              </Button>
            </Link>
          ) : (
            <Link to="/my-account/coupons" onClick={onClose}>
              <Button className="w-full">
                <Ticket size={16} /> Xem coupon của tôi
              </Button>
            </Link>
          )}
