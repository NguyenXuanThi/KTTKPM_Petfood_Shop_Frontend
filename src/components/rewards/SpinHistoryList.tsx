    ...
    <div className="overflow-hidden rounded-[1.5rem] border border-gray-100 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
      ...
      <div className="divide-y divide-gray-100 dark:divide-gray-800">
        {history.map((item) => (
          <div
            key={item._id}
            className="grid gap-3 p-4 md:grid-cols-[1.4fr_0.7fr_0.7fr_0.8fr] md:items-center md:px-5"
          >
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {item.rewardType === "coin" ? (
                ...
                <Badge variant="success">
                  <Gift size={12} /> {t("pawmart.rewardsPages.coupon")}
                </Badge>
              )}
            </div>
            <div>
            ...
            <div className="hidden text-sm text-gray-500 dark:text-gray-400 md:block">
              {new Date(item.playedAt || item.createdAt || "").toLocaleString(
                locale,
              )}
            </div>
          </div>
