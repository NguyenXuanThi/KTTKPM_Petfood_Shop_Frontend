...
function ItemFlags({ item }: { item: CartApiItem }) {
  ...
  if (!flags.priceChanged && !flags.outOfStock && !flags.inactiveProduct)
    return null;
...
export default function CartPage() {
  ...
  const { items, totals, totalItems, remove, updateQty, clear, isLoading } =
    useCartApi();
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (selectedItems.length === 0) {
        ...
        import("sonner").then(({ toast }) => {
          toast.warning(
            t("pawmart.cartExt.selectedNeedAttention", {
      ...
      import("sonner").then(({ toast }) =>
        toast.error(t("pawmart.cartExt.validateFailed")),
      );
    } finally {
  ...
  const shipping =
    selectedSubtotal >= 500_000 || selectedSubtotal === 0 ? 0 : 30_000;
  const orderTotal = selectedSubtotal + shipping;
  const hasIssues = items.some(
    (i) =>
      i.flags.priceChanged || i.flags.outOfStock || i.flags.inactiveProduct,
  );
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 md:px-6">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-amber-100 bg-amber-50/70 px-4 py-3 text-sm dark:border-amber-900/40 dark:bg-amber-900/20">
        ...
        <span className="text-gray-600 dark:text-gray-300">
          {t("pawmart.cartExt.selectedCheckout", {
                ...
                >
                  <div className="flex items-center">
                    <input
                      ...
                      className="h-4 w-4 rounded border-gray-300 text-amber-500 focus:ring-amber-400"
                      aria-label={t("pawmart.cartExt.selectForCheckout", {
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1 rounded-xl border border-gray-200 bg-white p-1 dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <button
                          ...
                          disabled={
                            item.flags.outOfStock || item.flags.inactiveProduct
                          }
                          className="flex h-7 w-7 items-center justify-center rounded-lg text-gray-500 transition-colors hover:bg-gray-100 disabled:opacity-40 dark:text-gray-400 dark:hover:bg-gray-700"
                        >
                          <Plus size={12} />
          ...
          <div className="sticky top-24 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                ...
                <span
                  className={
                    shipping === 0 ? "font-medium text-emerald-500" : ""
                  }
                >
                  {shipping === 0
                    ? t("pawmart.common.free")
                    : formatPrice(shipping)}
                </span>
              </div>
              {shipping > 0 && (
                <p className="text-xs text-amber-600 dark:text-amber-400">
                  {t("pawmart.cartExt.addMoreFreeShipping", {
                    amount: formatPrice(500_000 - selectedSubtotal),
                  })}
                </p>
              )}
            ...
            <Button
              ...
              disabled={
                (selectedItems.length === 0 ||
                  (selectedHasIssues && !isValidating)) &&
                !isValidating
              }
            >
              {isValidating ? (
                t("pawmart.cartExt.checkingOut")
              ) : (
                <>
